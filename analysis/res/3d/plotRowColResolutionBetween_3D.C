...
// Between-pixel bands (gaps) version of row/column resolution plot using 3D deltas
void plotRowColResolutionBetween_3DCore(const char* rootFilePath = "epicChargeSharing.root",
                                        const char* outRows = "row_resolution_between_3d.svg",
                                        const char* outCols = "col_resolution_between_3d.svg",
                                        bool saveImage = true,
                                        Long64_t seed = 0,
                                        int forceI0 = -1,
                                        int forceJ0 = -1,
                                        double edgeMarginMm = 0.1) // exclude ±100 µm near each pixel edge inside the gap
{
  ...
  // Effective half-band after excluding a fixed margin from each pixel edge towards the gap center
  const double halfBand  = std::max(1e-6, halfGap - std::max(0.0, edgeMarginMm));

  // Close / Far thresholds (in mm)
  const double closeMin = 0.05;  // 50 µm
  const double closeMax = 0.15;  // 150 µm
  const double farMin   = 0.15;  // 150 µm
  const double farMax   = 0.25;  // 250 µm
  ...
  std::vector<double> rowX_a_close, rowR_a_close, rowX_a_far, rowR_a_far; // yG0
  std::vector<double> rowX_b_close, rowR_b_close, rowX_b_far, rowR_b_far; // yG1
  std::vector<double> colY_a_close, colR_a_close, colY_a_far, colR_a_far; // xG0
  std::vector<double> colY_b_close, colR_b_close, colY_b_far, colR_b_far; // xG1
  ...
  TCanvas* cRows = makeCanvas("c_row_resolution_between_3d");
  ...
  // leave 4th pad empty intentionally for symmetry
  ...
  TCanvas* cCols = makeCanvas("c_col_resolution_between_3d");
...
void plotRowColResolutionBetween_3D() {
  ...
  plotRowColResolutionBetween_3DCore(defaultPath.c_str());
}

void plotRowColResolutionBetween_3D(const char* filename) {
  plotRowColResolutionBetween_3DCore(filename, "row_resolution_between_3d.svg", "col_resolution_between_3d.svg", true);
}
