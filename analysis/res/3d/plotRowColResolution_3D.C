...
//
// plotRowColResolution_3DCore
// Selects a random 3x3 like plotRandom3x3_3D_abs.C and builds two canvases:
//  - Rows: three horizontal bands (through pixel centers, band height = pixel size)
//  - Columns: three vertical bands (band width = pixel size)
// For each band we draw resolution vs position scatter:
//   rows:  x vs |ReconTrueDeltaX_3D|
//   cols:  y vs |ReconTrueDeltaY_3D|P
// Inside pixel interiors we overlay a constant line at ell/sqrt(12).
// We also add a 4th pad with the aggregated (mean) set across the three bands.
//
void plotRowColResolution_3DCore(const char* rootFilePath = "epicChargeSharing.root",
                                 const char* outRows = "row_resolution_3d.svg",
                                 const char* outCols = "col_resolution_3d.svg",
                                 bool saveImage = true,
                                 Long64_t seed = 0,
                                 int forceI0 = -1,
                                 int forceJ0 = -1)
{
  ...
  std::vector<double> hitDX; // |Δx| 3D
  std::vector<double> hitDY; // |Δy| 3D
  {
    ...
    for (Long64_t i = 0; i < nEntries; ++i) {
      ...
      hitDX.push_back(std::abs(dtx3d));
      hitDY.push_back(std::abs(dty3d));
    }
  }
  ...
  // Helper to draw a 1D scatter with shaded pixel regions and horizontal sigma=ℓ/√12 lines
  auto draw1D = [&](TPad* pad,
                    const char* title,
                    const char* xTitle,
                    double axisMin,
                    double axisMax,
                    const std::vector<std::pair<double,double>>& pixelRanges, // [x1,x2] triplet
    ...
    // Shaded pixel interiors (pink) and constant sigma line per interior
  ...
  // Build pixel interior ranges for x and y
  ...
  const double halfH = pixelSizeMm/2.0; // band half-height/half-width
  ...
  // ROWS canvas
  TCanvas* cRows = makeCanvas("c_row_resolution_3d");
  ...
  // COLUMNS canvas
  TCanvas* cCols = makeCanvas("c_col_resolution_3d");
  auto makeYRangesTitle = [&](double yC) { return Form("Column x=%.3f mm", yC); };
...
// Convenience wrappers (execute when running this file directly)
void plotRowColResolution_3D() {
  ...
  plotRowColResolution_3DCore(defaultPath.c_str());
}

void plotRowColResolution_3D(const char* filename) {
  plotRowColResolution_3DCore(filename, "row_resolution_3d.svg", "col_resolution_3d.svg", true);
}
