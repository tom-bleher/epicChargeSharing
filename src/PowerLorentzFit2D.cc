...
bool PowerLorentzCeres(
    ...
    if (x_vals.size() != y_vals.size() || x_vals.size() < 5) {
        if (verbose) {
            std::cout << "Insufficient data points for Power Lorentz fitting, using emergency fallback" << std::endl;
        }
        
        // EMERGENCY FALLBACK: Use simple statistical estimates
        if (x_vals.size() >= 2) {
            DataStatistics emergency_stats = CalcRobustStatisticsPowerLorentz(x_vals, y_vals);
            if (emergency_stats.valid) {
                ...
                if (verbose) {
                    std::cout << "Emergency fallback Power Lorentz fit applied" << std::endl;
                }
        ...
        if (verbose) {
            std::cout << "Ultimate fallback Power Lorentz fit applied" << std::endl;
        }
        return true; // Always return success
    }
    ...
    if (verbose) {
        std::cout << "All sophisticated Power Lorentz fitting strategies failed, using robust statistical fallback" << std::endl;
    }
    ...
    if (!filtered_datasets.empty()) {
        ...
        if (fallback_x.size() < 5 && filtered_datasets.size() > 1) {
            fallback_x = filtered_datasets[1].first;
            fallback_y = filtered_datasets[1].second;
        }
    }
    
    DataStatistics fallback_stats = CalcRobustStatisticsPowerLorentz(fallback_x, fallback_y);
    if (fallback_stats.valid && fallback_x.size() >= 2) {
        // Statistical parameter estimation for Power Lorentz
        ...
        // Estimate gamma from data spread (Power Lorentz has adjustable tails)
        ...
        if (weight_sum > 0) {
            fit_gamma = std::sqrt(2.0 * weighted_gamma / weight_sum); // Power Lorentz width
        } else {
        ...
        if (verbose) {
            std::cout << "Robust statistical fallback Power Lorentz fit applied" << std::endl;
        }
    ...
    if (verbose) {
        std::cout << "Emergency heuristic Power Lorentz fit applied - all other methods failed" << std::endl;
    }
