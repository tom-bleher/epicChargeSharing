...
void InitializeROOT() {
    ...
    if (!gROOTInitialized.load()) {
        ...
        gROOTInitialized.store(true);
    }
}

// Robust statistics calculations for parameter estimation
struct DataStatistics {
    ...
    DataStatistics() : mean(0), median(0), std_dev(0), mad(0), 
                      q25(0), q75(0), min_val(0), max_val(0),
                      weighted_mean(0), total_weight(0), valid(false) {}
};

DataStatistics CalculateRobustStatisticsROOT(const std::vector<double>& x_vals, 
...
// Parameter estimation with multiple strategies
struct ParameterEstimates {
    double amplitude, center, sigma, offset;
    double amplitude_err, center_err, sigma_err, offset_err;
    bool valid;
    int method_used;
    
    ParameterEstimates() : amplitude(0), center(0), sigma(0), offset(0),
                          amplitude_err(0), center_err(0), sigma_err(0), offset_err(0),
                          valid(false), method_used(0) {}
};

ParameterEstimates EstimateGaussianParametersROOT(
    ...
    DataStatistics stats = CalculateRobustStatisticsROOT(x_vals, y_vals);
    if (!stats.valid) {
        return estimates;
    }
    
    if (verbose) {
        std::cout << "Gaussian data statistics: min=" << stats.min_val << ", max=" << stats.max_val 
    ...
    estimates.offset = std::min({stats.min_val, stats.q25 * 0.5, charge_scale * 0.001});
    estimates.offset = std::max(estimates.offset, 0.0); // Ensure non-negative
    ...
    // Better sigma estimation based on data spread and physics
    // For charge distributions, start with reasonable physical width
    estimates.sigma = pixel_spacing * 0.3; // Conservative starting point
    
    // Refine using weighted second moment, but with constraints
    double weighted_second_moment = 0.0;
    double total_weight = 0.0;
    double amplitude_threshold = estimates.offset + estimates.amplitude * 0.1; // 10% above baseline
    
    for (size_t i = 0; i < x_vals.size(); ++i) {
        if (y_vals[i] > amplitude_threshold) {
            double weight = y_vals[i] - estimates.offset;
    ...
    if (total_weight > 0) {
        double moment_sigma = std::sqrt(weighted_second_moment / total_weight);
        // Constrain sigma to reasonable physical range
        moment_sigma = std::max(moment_sigma, pixel_spacing * 0.05); // Minimum: 5% of pixel spacing
        moment_sigma = std::min(moment_sigma, pixel_spacing * 2.5);  // Maximum: 2.5x pixel spacing
        
        // Use average of initial estimate and moment-based estimate
        estimates.sigma = (estimates.sigma + moment_sigma) / 2.0;
    }
    
    // Final constraints on sigma
    estimates.sigma = std::max(estimates.sigma, pixel_spacing * 0.05);
    estimates.sigma = std::min(estimates.sigma, pixel_spacing * 3.0);
    
    // Error estimates proportional to signal magnitude
    estimates.amplitude_err = estimates.amplitude * 0.05; // 5% relative error
    estimates.center_err = pixel_spacing * 0.02; // 2% of pixel spacing
    estimates.sigma_err = estimates.sigma * 0.10; // 10% relative error for width
    estimates.offset_err = charge_scale * 0.001; // Small absolute error for baseline
    
    estimates.valid = true;
    estimates.method_used = 1;
    
    if (verbose) {
        std::cout << "Gaussian parameter estimates: A=" << estimates.amplitude 
...
std::vector<bool> FilterOutliers(const std::vector<double>& x_vals,
                                const std::vector<double>& y_vals,
...
// Core Gaussian fitting function using ROOT
struct FitResult {
    double amplitude, center, sigma, offset;
    double amplitude_err, center_err, sigma_err, offset_err;
    ...
    FitResult() : amplitude(0), center(0), sigma(0), offset(0),
                 amplitude_err(0), center_err(0), sigma_err(0), offset_err(0),
                 chi2(0), reduced_chi2(0), prob(0), ndf(0), fit_status(-1),
                 edm(0), fwhm(0), successful(false) {}
};

FitResult FitGaussianROOT(const std::vector<double>& x_vals,
                         const std::vector<double>& y_vals,
                         const ParameterEstimates& initial_params,
                         double pixel_spacing = 0.5,
    ...
    try {
        ...
        int fit_id = ++gGlobalROOTFitCounter;
        std::string func_name = "gauss_fit_" + std::to_string(fit_id) + "_" + 
                               std::to_string(std::hash<std::thread::id>{}(std::this_thread::get_id()));
        
        // Define Gaussian function with improved syntax
        ...
        // Use simpler, more robust TFormula syntax
        std::string formula = "[0]*exp(-0.5*pow((x-[1])/[2],2)) + [3]";
        ...
        func->SetParameter(2, initial_params.sigma);
        func->SetParameter(3, initial_params.offset);
        
        // Set parameter limits appropriate for extremely small charge values
        double data_magnitude = std::max(initial_params.amplitude, 1e-18);
        double pixel_scale = pixel_spacing; // 0.5mm typically
        
        // Amplitude: allow wide range but keep physical
        func->SetParLimits(0, data_magnitude * 0.001, data_magnitude * 1000);
        
        // Center: constrain to reasonable range around data
        func->SetParLimits(1, x_min - pixel_scale, x_max + pixel_scale);
        
        // Sigma: physical constraints based on pixel spacing (0.025 to 1.5 mm typically)
        func->SetParLimits(2, pixel_scale * 0.05, pixel_scale * 3.0);
        
        // Offset: should be small and non-negative for charge measurements
        func->SetParLimits(3, 0.0, data_magnitude * 0.1);
        
        // Create TGraphErrors for fitting
        std::string graph_name = "graph_" + std::to_string(fit_id) + "_" + 
                                std::to_string(std::hash<std::thread::id>{}(std::this_thread::get_id()));
        ...
        if (use_weighted_fit) {
            ...
            double relative_error = 0.05; // 5% relative error as baseline
            
            for (size_t i = 0; i < y_vals.size(); ++i) {
                // Use relative error for small signals, with minimum based on data scale
                double min_error = data_scale * 0.01; // 1% of maximum as minimum error
        ...
        if (fit_result.Get() && fit_result->IsValid()) {
            ...
            result.sigma = func->GetParameter(2);
            result.offset = func->GetParameter(3);
            
            result.amplitude_err = func->GetParError(0);
            result.center_err = func->GetParError(1);
            result.sigma_err = func->GetParError(2);
            result.offset_err = func->GetParError(3);
            ...
            // Calculate FWHM: FWHM = 2.35482 * sigma for Gaussian
            result.fwhm = 2.35482 * std::abs(result.sigma);
            
            // Check fit quality with criteria appropriate for small values
            double data_magnitude = std::max(result.amplitude, 1e-18);
            result.successful = (result.fit_status == 0) && 
                              (result.reduced_chi2 > 0) &&
                              (result.reduced_chi2 < 200) && // More lenient chi2 for small values
                              (result.sigma > 0) &&
                              (result.amplitude > 0) &&
                              (result.amplitude_err < 15 * result.amplitude) && // More lenient for small signals
                              (result.center_err < range * 0.3) && // Position should be reasonably precise  
                              (result.sigma_err < 15 * result.sigma) && // Width can be less precise
                              (result.amplitude > data_magnitude * 0.01); // Amplitude should be significant
            
            if (verbose) {
                std::cout << "ROOT Gaussian fit results:" << std::endl;
                ...
                std::cout << "  Sigma: " << result.sigma << " ± " << result.sigma_err << std::endl;
                std::cout << "  FWHM: " << result.fwhm << std::endl;
                std::cout << "  Offset: " << result.offset << " ± " << result.offset_err << std::endl;
        ...
        if (verbose) {
            std::cout << "ROOT fitting error: " << e.what() << std::endl;
        }
    }
...
// Group data by row/column for 2D fitting
void GroupDataByRow(const std::vector<double>& x_coords,
...
void GroupDataByColumn(const std::vector<double>& x_coords,
                      const std::vector<double>& y_coords,
                      const std::vector<double>& charge_values,
                      double center_x,
                      double pixel_spacing,
...
// Main 2D Gaussian fitting function
GaussianFit2DResultsROOT Fit2DGaussianROOT(
    ...
    GaussianFit2DResultsROOT results;
    ...
    try {
        ...
        if (enable_outlier_filtering) {
            auto outlier_mask = FilterOutliers(x_coords, charge_values);
            filtered_x.clear();
            filtered_y.clear();
        ...
        GroupDataByRow(filtered_x, filtered_y, filtered_charge, 
        ...
        GroupDataByColumn(filtered_x, filtered_y, filtered_charge, 
        ...
        if (row_x.size() >= 4) {
                    auto x_params = EstimateGaussianParametersROOT(row_x, row_charge, 
                                                      center_x_estimate, pixel_spacing, verbose);
            if (x_params.valid) {
                auto x_result = FitGaussianROOT(row_x, row_charge, x_params, 
                                              pixel_spacing, minimizer, max_iterations, tolerance, 
                                              use_weighted_fit, verbose);
                
                if (x_result.successful) {
                    results.x_center = x_result.center;
                    results.x_sigma = x_result.sigma;
                    results.x_amplitude = x_result.amplitude;
                    results.x_center_err = x_result.center_err;
                    results.x_sigma_err = x_result.sigma_err;
                    results.x_amplitude_err = x_result.amplitude_err;
                    results.x_vertical_offset = x_result.offset;
                    results.x_vertical_offset_err = x_result.offset_err;
        ...
        if (col_y.size() >= 4) {
                        auto y_params = EstimateGaussianParametersROOT(col_y, col_charge, 
                                                          center_y_estimate, pixel_spacing, verbose);
            if (y_params.valid) {
                auto y_result = FitGaussianROOT(col_y, col_charge, y_params, 
                                              pixel_spacing, minimizer, max_iterations, tolerance, 
                                              use_weighted_fit, verbose);
                
                if (y_result.successful) {
                    results.y_center = y_result.center;
                    results.y_sigma = y_result.sigma;
                    results.y_amplitude = y_result.amplitude;
                    results.y_center_err = y_result.center_err;
                    results.y_sigma_err = y_result.sigma_err;
                    results.y_amplitude_err = y_result.amplitude_err;
                    results.y_vertical_offset = y_result.offset;
                    results.y_vertical_offset_err = y_result.offset_err;
        ...
        if (verbose) {
            std::cout << "2D Gaussian fit completed. X fit: " << (x_fit_successful ? "SUCCESS" : "FAILED")
                     << ", Y fit: " << (y_fit_successful ? "SUCCESS" : "FAILED") << std::endl;
        }
        
    } catch (const std::exception& e) {
        if (verbose) {
            std::cout << "Error in 2D Gaussian fitting: " << e.what() << std::endl;
        }
    }
...
// Diagonal data grouping
void GroupDataByMainDiagonal(const std::vector<double>& x_coords,
                            const std::vector<double>& y_coords,
                            const std::vector<double>& charge_values,
...
void GroupDataBySecondaryDiagonal(const std::vector<double>& x_coords,
                                 const std::vector<double>& y_coords,
                                 const std::vector<double>& charge_values,
                                 double center_x, double center_y,
                                 double pixel_spacing,
...
// Diagonal Gaussian fitting function
DiagonalFitResultsROOT FitDiagonalGaussianROOT(
    ...
    DiagonalFitResultsROOT results;
    ...
    try {
        ...
        GroupDataByMainDiagonal(filtered_x, filtered_y, filtered_charge,
        ...
        GroupDataBySecondaryDiagonal(filtered_x, filtered_y, filtered_charge,
                                    center_x_estimate, center_y_estimate, pixel_spacing,
        ...
        if (main_diag_x.size() >= 4) {
                        auto params = EstimateGaussianParametersROOT(main_diag_x, main_diag_charge, 
                                                        center_x_estimate, pixel_spacing, verbose);
            if (params.valid) {
                auto fit_result = FitGaussianROOT(main_diag_x, main_diag_charge, params, 
                                                pixel_spacing, minimizer, max_iterations, tolerance, 
                                                use_weighted_fit, verbose);
                
                if (fit_result.successful) {
                    results.main_diag_x_center = fit_result.center;
                    results.main_diag_x_sigma = fit_result.sigma;
                    results.main_diag_x_amplitude = fit_result.amplitude;
                    results.main_diag_x_center_err = fit_result.center_err;
                    results.main_diag_x_sigma_err = fit_result.sigma_err;
                    results.main_diag_x_amplitude_err = fit_result.amplitude_err;
                    results.main_diag_x_vertical_offset = fit_result.offset;
                    results.main_diag_x_vertical_offset_err = fit_result.offset_err;
        ...
        if (main_diag_y.size() >= 4) {
            auto params = EstimateGaussianParametersROOT(main_diag_y, main_diag_charge, 
                                                        center_y_estimate, pixel_spacing, verbose);
            if (params.valid) {
                auto fit_result = FitGaussianROOT(main_diag_y, main_diag_charge, params, 
                                                pixel_spacing, minimizer, max_iterations, tolerance, 
                                                use_weighted_fit, verbose);
                
                if (fit_result.successful) {
                    results.main_diag_y_center = fit_result.center;
                    results.main_diag_y_sigma = fit_result.sigma;
                    results.main_diag_y_amplitude = fit_result.amplitude;
                    results.main_diag_y_center_err = fit_result.center_err;
                    results.main_diag_y_sigma_err = fit_result.sigma_err;
                    results.main_diag_y_amplitude_err = fit_result.amplitude_err;
                    results.main_diag_y_vertical_offset = fit_result.offset;
                    results.main_diag_y_vertical_offset_err = fit_result.offset_err;
        ...
        if (sec_diag_x.size() >= 4) {
            auto params = EstimateGaussianParametersROOT(sec_diag_x, sec_diag_charge, 
                                                        center_x_estimate, pixel_spacing, verbose);
            if (params.valid) {
                auto fit_result = FitGaussianROOT(sec_diag_x, sec_diag_charge, params, 
                                                pixel_spacing, minimizer, max_iterations, tolerance, 
                                                use_weighted_fit, verbose);
                
                if (fit_result.successful) {
                    results.sec_diag_x_center = fit_result.center;
                    results.sec_diag_x_sigma = fit_result.sigma;
                    results.sec_diag_x_amplitude = fit_result.amplitude;
                    results.sec_diag_x_center_err = fit_result.center_err;
                    results.sec_diag_x_sigma_err = fit_result.sigma_err;
                    results.sec_diag_x_amplitude_err = fit_result.amplitude_err;
                    results.sec_diag_x_vertical_offset = fit_result.offset;
                    results.sec_diag_x_vertical_offset_err = fit_result.offset_err;
        ...
        if (sec_diag_y.size() >= 4) {
            auto params = EstimateGaussianParametersROOT(sec_diag_y, sec_diag_charge, 
                                                        center_y_estimate, pixel_spacing, verbose);
            if (params.valid) {
                auto fit_result = FitGaussianROOT(sec_diag_y, sec_diag_charge, params, 
                                                pixel_spacing, minimizer, max_iterations, tolerance, 
                                                use_weighted_fit, verbose);
                
                if (fit_result.successful) {
                    results.sec_diag_y_center = fit_result.center;
                    results.sec_diag_y_sigma = fit_result.sigma;
                    results.sec_diag_y_amplitude = fit_result.amplitude;
                    results.sec_diag_y_center_err = fit_result.center_err;
                    results.sec_diag_y_sigma_err = fit_result.sigma_err;
                    results.sec_diag_y_amplitude_err = fit_result.amplitude_err;
                    results.sec_diag_y_vertical_offset = fit_result.offset;
                    results.sec_diag_y_vertical_offset_err = fit_result.offset_err;
        ...
        if (verbose) {
            std::cout << "Diagonal Gaussian fits completed:" << std::endl;
        ...
        if (verbose) {
            std::cout << "Error in diagonal Gaussian fitting: " << e.what() << std::endl;
        }
    }
