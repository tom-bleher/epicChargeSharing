...
bool Fit3DLorentzianCeres(
    ...
    for (size_t dataset_idx = 0; dataset_idx < filtered_datasets.size(); ++dataset_idx) {
        ...
        auto try_config = [&](const Lorentzian3DFittingConfig& config, const std::string& stage_name) -> bool {
            ...
            if (any_success) {
                if (verbose) {
                    std::cout << "Base 3D Lorentzian fit χ²ᵣ=" << best_chi2_reduced << ", trying perturbations..." << std::endl;
                }
        ...
        if (verbose) {
            std::cout << "Cheap 3D Lorentzian config " << (success ? "succeeded" : "failed") 
        ...
        if (verbose) {
            std::cout << "Trying all " << expensive_configs.size() 
                     << " expensive 3D Lorentzian configurations..." << std::endl;
        }
        ...
        for (size_t i = 0; i < expensive_configs.size(); ++i) {
            ...
            if (verbose) {
                std::cout << "Expensive 3D Lorentzian config " << (i+1) << " " 
        ...
        if (best_success) {
            if (verbose) {
                std::cout << "Best 3D Lorentzian fit achieved with χ²ᵣ=" << best_chi2 << std::endl;
            }
            return true;
        }
    }
