...
// 3D Lorentz cost function with err (5% of max charge)
// Function form: z(x,y) = A / (1 + ((x - mx) / γx)^2 + ((y - my) / γy)^2) + B  
struct Lorentz3DCostFunction {
    Lorentz3DCostFunction(double x, double y, double z, double err) 
    ...
    bool operator()(const T* const params, T* residual) const {
        ...
        // 3D Lorentz function: z(x,y) = A / (1 + ((x - mx) / γx)^2 + ((y - my) / γy)^2) + B
        T dx = x_ - mx;
        T dy = y_ - my;
    ...
    static ceres::CostFunction* Create(double x, double y, double z, double err) {
        return (new ceres::AutoDiffCostFunction<Lorentz3DCostFunction, 1, 6>(
            new Lorentz3DCostFunction(x, y, z, err)));
    }
...
// Parameter estimation structures for 3D Lorentz
struct Lorentz3DParameterEstimates {
    double amp;
    double center_x;
    double center_y;
    double gamma_x;
...
Data3DStatistics CalcRobust3DStatistics(const std::vector<double>& x_vals, 
...
// Parameter estimation for 3D Lorentz distributions
Lorentz3DParameterEstimates Estimate3DLorentzParameters(
    ...
    Lorentz3DParameterEstimates estimates;
    ...
    if (verbose) {
        std::cout << "3D Lorentz data statistics: min=" << stats.min_val << ", max=" << stats.max_val 
    ...
    // For 3D Lorentz: gamma estimation based on charge spread in both directions
    ...
    if (weight_sum > 0) {
        ...
        estimates.gamma_x = pixel_spacing * 0.7; // Larger default for Lorentz
    ...
    if (estimates.amp > 0 && estimates.gamma_x > 0 && estimates.gamma_y > 0 &&
        ...
        if (verbose) {
            std::cout << "3D Lorentz Method 1 (Physics-based): A=" << estimates.amp 
    ...
    if (estimates.amp > 0 && estimates.gamma_x > 0 && estimates.gamma_y > 0) {
        ...
        if (verbose) {
            std::cout << "3D Lorentz Method 2 (Robust statistical): A=" << estimates.amp 
                     << ", mx=" << estimates.center_x << ", my=" << estimates.center_y
                     << ", gamma_x=" << estimates.gamma_x << ", gamma_y=" << estimates.gamma_y
                     << ", B=" << estimates.baseline << std::endl;
        }
    ...
    if (verbose) {
        std::cout << "3D Lorentz Method 3 (Conservative fallback): A=" << estimates.amp 
                 << ", mx=" << estimates.center_x << ", my=" << estimates.center_y
                 << ", gamma_x=" << estimates.gamma_x << ", gamma_y=" << estimates.gamma_y
                 << ", B=" << estimates.baseline << std::endl;
    }
...
// Outlier filtering for 3D Lorentz fitting (adapted from 2D version)
std::tuple<std::vector<double>, std::vector<double>, std::vector<double>> Filter3DLorentzOutliers(
    const std::vector<double>& x_vals,
    const std::vector<double>& y_vals,
    const std::vector<double>& z_vals,
    ...
    if (filtered_x.size() < x_vals.size() / 2) {
        if (verbose) {
            std::cout << "Too many 3D Lorentz outliers detected (" << outliers_removed 
    ...
    if (filtered_x.size() < 6) {
        if (verbose) {
            std::cout << "Warning: After 3D Lorentz outlier filtering, only " << filtered_x.size() 
    ...
    if (verbose && outliers_removed > 0) {
        std::cout << "Removed " << outliers_removed << " 3D Lorentz outliers, " 
...
// Core 3D Lorentz fitting function using Ceres Solver
bool LorentzCeres3D_detail(
    const std::vector<double>& x_vals,
    const std::vector<double>& y_vals,
    const std::vector<double>& z_vals,
    double center_x_estimate,
    ...
    if (x_vals.size() != y_vals.size() || x_vals.size() != z_vals.size() || x_vals.size() < 6) {
        if (verbose) {
            std::cout << "Insufficient data points for 3D Lorentz fitting, using emergency fallback" << std::endl;
        }
        ...
        if (x_vals.size() >= 3) {
            ...
            if (emergency_stats.valid) {
                ...
                if (verbose) {
                    std::cout << "Emergency fallback 3D Lorentz fit applied" << std::endl;
                }
        ...
        if (verbose) {
            std::cout << "Ultimate fallback 3D Lorentz fit applied" << std::endl;
        }
        return true; // Always return success
    }
    ...
    if (enable_outlier_filtering) {
        auto conservative_data = Filter3DLorentzOutliers(x_vals, y_vals, z_vals, 2.5, verbose);
        if (std::get<0>(conservative_data).size() >= 6) {
            filtered_datasets.push_back(conservative_data);
        }
        
        auto lenient_data = Filter3DLorentzOutliers(x_vals, y_vals, z_vals, 3.0, verbose);
    ...
    if (verbose) {
        std::cout << "3D Lorentz outlier filtering " << (enable_outlier_filtering ? "enabled" : "disabled") 
    ...
    for (size_t dataset_idx = 0; dataset_idx < filtered_datasets.size(); ++dataset_idx) {
        ...
        if (verbose) {
            std::cout << "Trying 3D Lorentz dataset " << dataset_idx << " with " << clean_x.size() << " points" << std::endl;
        }
        
        // Get parameter estimates
        Lorentz3DParameterEstimates estimates = Estimate3DLorentzParameters(clean_x, clean_y, clean_z, center_x_estimate, center_y_estimate, pixel_spacing, verbose);
        if (!estimates.valid) {
            if (verbose) {
                std::cout << "3D Lorentz parameter estimation failed for dataset " << dataset_idx << std::endl;
            }
            continue;
        }
        ...
        auto try_config = [&](const Lorentz3DtingConfig& config, const std::string& stage_name) -> bool {
            if (verbose) {
                std::cout << "Trying 3D Lorentz " << stage_name << " configuration..." << std::endl;
            }
            // STEP 2 OPTIMIZATION: Hierarchical multi-start budget for 3D Lorentz
            ...
            for (const auto& guess : initial_guesses) {
                ...
                for (size_t i = 0; i < clean_x.size(); ++i) {
                    ceres::CostFunction* cost_function = Lorentz3DCostFunction::Create(
                ...
                if (fit_success) {
                    ...
                    if (cost < best_cost) {
                        ...
                        if (verbose) {
                            std::cout << "New best 3D Lorentz result from " << guess.description 
            ...
            if (any_success) {
                if (verbose) {
                    std::cout << "Base 3D Lorentz fit χ²ᵣ=" << best_chi2_reduced << ", trying perturbations..." << std::endl;
                }
            ...
            if (any_success) {
                ...
                if (verbose) {
                    std::cout << "Success 3D Lorentz fit with " << stage_name 
        ...
        if (verbose) {
            std::cout << "Cheap 3D Lorentz config " << (success ? "succeeded" : "failed") 
        ...
        if (verbose) {
            std::cout << "Trying all " << expensive_configs.size() 
                     << " expensive 3D Lorentz configurations..." << std::endl;
        }
        ...
        for (size_t i = 0; i < expensive_configs.size(); ++i) {
            ...
            if (verbose) {
                std::cout << "Expensive 3D Lorentz config " << (i+1) << " " 
        ...
        if (best_success) {
            if (verbose) {
                std::cout << "Best 3D Lorentz fit achieved with χ²ᵣ=" << best_chi2 << std::endl;
            }
            return true;
        }
    }
    ...
    if (verbose) {
        std::cout << "All sophisticated 3D Lorentz fitting strategies failed, using robust statistical fallback" << std::endl;
    }
    ...
    if (fallback_stats.valid && fallback_x.size() >= 3) {
        // Statistical parameter estimation for 3D Lorentz
        ...
        // Estimate gammas from data spread (Lorentz has wider tails than Gauss)
        ...
        if (weight_sum > 0) {
            fit_gamma_x = std::sqrt(2.0 * weighted_gamma_x / weight_sum); // Lorentz is wider than Gauss
        ...
        if (verbose) {
            std::cout << "Robust statistical fallback 3D Lorentz fit applied" << std::endl;
        }
    ...
    if (verbose) {
        std::cout << "Emergency heuristic 3D Lorentz fit applied - all other methods failed" << std::endl;
    }
...
Lorentz3DResultsCeres LorentzCeres3D(
    ...
    bool enable_outlier_filtering)
{
    Lorentz3DResultsCeres result;
    
    // Initialize Ceres logging (removed mutex for better parallelization)
    InitializeCeres3DLorentz();
    
    if (x_coords.size() != y_coords.size() || x_coords.size() != charge_values.size()) {
        if (verbose) {
            std::cout << "LorentzCeres3D: Error - coordinate and charge vector sizes don't match" << std::endl;
        }
        return result;
    }
    ...
    if (x_coords.size() < 6) {
        if (verbose) {
            std::cout << "LorentzCeres3D: Error - need at least 6 data points for 3D fitting" << std::endl;
        }
        return result;
    }
    
    if (verbose) {
        std::cout << "Starting 3D Lorentz fit (Ceres) with " << x_coords.size() << " data points" << std::endl;
    }
    ...
    // Perform 3D Lorentz surface fitting
    bool fit_success = LorentzCeres3D_detail(
        x_coords, y_coords, charge_values, center_x_estimate, center_y_estimate, pixel_spacing,
        result.amp, result.center_x, result.center_y, result.gamma_x, result.gamma_y, result.vert_offset,
        result.amp_err, result.center_x_err, result.center_y_err, result.gamma_x_err, result.gamma_y_err, result.vert_offset_err,
    ...
    if (verbose) {
        std::cout << "3D Lorentz fit (Ceres) " << (result.fit_success ? "success" : "failed") << std::endl;
    }
