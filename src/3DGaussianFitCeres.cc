...
bool Fit3DGaussianCeres(
    ...
    for (size_t dataset_idx = 0; dataset_idx < filtered_datasets.size(); ++dataset_idx) {
        ...
        auto try_config = [&](const Gaussian3DFittingConfig& config, const std::string& stage_name) -> bool {
            ...
            if (any_success) {
                if (verbose) {
                    std::cout << "Base 3D Gaussian fit χ²ᵣ=" << best_chi2_reduced << ", trying perturbations..." << std::endl;
                }
        ...
        if (verbose) {
            std::cout << "Cheap 3D Gaussian config " << (success ? "succeeded" : "failed") 
        ...
        if (verbose) {
            std::cout << "Trying all " << expensive_configs.size() 
                     << " expensive 3D Gaussian configurations..." << std::endl;
        }
        ...
        for (size_t i = 0; i < expensive_configs.size(); ++i) {
            ...
            if (verbose) {
                std::cout << "Expensive 3D Gaussian config " << (i+1) << " " 
        ...
        if (best_success) {
            if (verbose) {
                std::cout << "Best 3D Gaussian fit achieved with χ²ᵣ=" << best_chi2 << std::endl;
            }
            return true;
        }
    }
