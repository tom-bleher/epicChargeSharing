...
static std::atomic<int> gGlobalCeres3DLorentzCounter{0};
...
bool LorentzCeres3D_detail(
    ...
    if (x_vals.size() != y_vals.size() || x_vals.size() != z_vals.size() || x_vals.size() < 6) {
        if (verbose) {
            std::cout << "Insufficient data points for 3D Lorentz fitting" << std::endl;
        }
        return false;
    }
    ...
    if (verbose) {
        std::cout << "All 3D Lorentz fitting strategies failed" << std::endl;
    }
    return false;
}
