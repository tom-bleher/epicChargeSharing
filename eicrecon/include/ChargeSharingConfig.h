...
struct ChargeSharingConfig {
    ...
    double effectivePixelSizeYMM() const { return (pixelSizeYMM > 0.0) ? pixelSizeYMM : pixelSizeMM; }

    /// Get effective Y pixel spacing (uses X if Y not specified)
    double effectivePixelSpacingYMM() const { return (pixelSpacingYMM > 0.0) ? pixelSpacingYMM : pixelSpacingMM; }
    ...
    double sigmaLoBound() const { return pixelSizeMM; }

    double sigmaHiBound() const { return static_cast<double>(neighborhoodRadius) * pixelSpacingMM; }
