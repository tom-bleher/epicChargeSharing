...
private:
    ...
    ParameterRef<int> m_activePixelMode{
        this, "activePixelMode", m_activePixelModeValue,
    ...
    ParameterRef<float> m_minEDep{this, "minEDep", config().minEDepGeV, "Minimum energy deposit threshold (GeV)"};
    ...
    ParameterRef<double> m_pixelSize{this, "pixelSizeMM", config().pixelSizeMM, "Pixel pad size (mm)"};
    ...
    ParameterRef<double> m_detectorSize{this, "detectorSizeMM", config().detectorSizeMM, "Sensor side length (mm)"};
    ParameterRef<double> m_detectorThickness{this, "detectorThicknessMM", config().detectorThicknessMM,
                                             "Silicon thickness (mm)"};
    ParameterRef<double> m_pixelThickness{this, "pixelThicknessMM", config().pixelThicknessMM, "Pixel depth (mm)"};
    ...
    ParameterRef<double> m_elementaryCharge{this, "elementaryChargeC", config().elementaryChargeC,
                                            "Elementary charge (C)"};
    ParameterRef<double> m_d0Micron{this, "d0Micron", config().d0Micron, "Transverse hit size d0 for LogA model (um)"};
    ...
    ParameterRef<double> m_fitDistanceScalePixels{this, "fitDistanceScalePixels", config().fitDistanceScalePixels,
                                                  "Distance scale in pixel units"};
    ParameterRef<double> m_fitDistanceExponent{this, "fitDistanceExponent", config().fitDistanceExponent,
                                               "Distance error power law exponent"};
    ParameterRef<double> m_fitDistanceFloorPercent{this, "fitDistanceFloorPercent", config().fitDistanceFloorPercent,
                                                   "Minimum fit error (% of Q_max)"};
    ParameterRef<double> m_fitDistanceCapPercent{this, "fitDistanceCapPercent", config().fitDistanceCapPercent,
