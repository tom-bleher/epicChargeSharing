...
inline double calcPadViewAngle(double distanceToCenterMM,
                               double padWidthMM,
                               double padHeightMM) {
...
inline double getLinearBeta(double pitchMM) {
  ...
  if (pitchUM >= constants::kLinearMinPitchUM &&
      pitchUM <= constants::kLinearBoundaryPitchUM) {
    return constants::kLinearBetaNarrow;
  }
  if (pitchUM > constants::kLinearBoundaryPitchUM &&
      pitchUM <= constants::kLinearMaxPitchUM) {
...
inline double calcWeight(SignalModel model,
                         double distanceMM,
                         double padWidthMM,
                         double padHeightMM,
                         double d0MM,
                         double betaPerMicron = 0.0,
                         double pitchMM = 0.0) {
...
struct NeighborhoodResult {
  ...
  std::vector<const NeighborPixel*> getCenterRow() const {
    ...
    std::sort(row.begin(), row.end(),
              [](const NeighborPixel* a, const NeighborPixel* b) {
                return a->di < b->di;
              });
    return row;
  }
...
inline NeighborhoodResult calculateNeighborhood(
  ...
  for (int di = -radius; di <= radius; ++di) {
    for (int dj = -radius; dj <= radius; ++dj) {
      ...
      pixel.inBounds = true;
      pixel.globalIndex = (config.numPixelsY > 0)
                          ? globalI * config.numPixelsY + globalJ
      ...
      pixel.weight = calcWeight(config.signalModel, pixel.distance,
                                padW, padH, d0MM, config.betaPerMicron, pitchX);
  ...
  switch (config.activeMode) {
    ...
    case ActivePixelMode::ChargeBlock3x3: {
      ...
      for (int oi = maxOffset; oi <= 0; ++oi) {
        for (int oj = maxOffset; oj <= 0; ++oj) {
          ...
          for (const auto& p : result.pixels) {
            if (p.inBounds &&
                p.di >= testCornerDi && p.di < testCornerDi + blockSize &&
      ...
      for (size_t i = 0; i < result.pixels.size(); ++i) {
        const auto& p = result.pixels[i];
        isActive[i] = p.inBounds &&
                      p.di >= bestCornerDi && p.di < bestCornerDi + blockSize &&
  ...
  for (auto& p : result.pixels) {
    if (p.inBounds &&
        p.di >= blockCornerDi && p.di < blockCornerDi + blockSize &&
        p.dj >= blockCornerDj && p.dj < blockCornerDj + blockSize &&
        bestBlockSum > 0.0) {
...
class NoiseModel {
 public:
  ...
  double applyNoise(double chargeC) {
    ...
    double gainSigma = m_uniformDist(m_generator) *
                       (m_config.gainSigmaMax - m_config.gainSigmaMin) +
                       m_config.gainSigmaMin;
    ...
    double noiseC = m_gaussDist(m_generator) *
                    m_config.electronNoiseCount *
                    m_config.elementaryCharge;
    noisyCharge += noiseC;
