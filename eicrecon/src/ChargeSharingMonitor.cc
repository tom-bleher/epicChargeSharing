...
void ChargeSharingMonitor::CreateHistograms(const std::string& detectorName) {
  ...
  hists.hResidualX = new TH1D("hResidualX",
                               fmt::format("{} X Residual;#Deltax = x_{{reco}} - x_{{true}} [mm];Counts",
                                           detectorName).c_str(),
                               200, -0.5, 0.5);

  hists.hResidualY = new TH1D("hResidualY",
                               fmt::format("{} Y Residual;#Deltay = y_{{reco}} - y_{{true}} [mm];Counts",
                                           detectorName).c_str(),
                               200, -0.5, 0.5);

  hists.hResidualR = new TH1D("hResidualR",
                               fmt::format("{} Radial Residual;#Deltar [mm];Counts",
                                           detectorName).c_str(),
                               100, 0.0, 0.5);

  // 2D correlation plots
  hists.hRecoVsTrueX = new TH2D("hRecoVsTrueX",
                                 fmt::format("{} Reco vs True X;x_{{true}} [mm];x_{{reco}} [mm]",
                                             detectorName).c_str(),
                                 100, -20, 20, 100, -20, 20);

  hists.hRecoVsTrueY = new TH2D("hRecoVsTrueY",
                                 fmt::format("{} Reco vs True Y;y_{{true}} [mm];y_{{reco}} [mm]",
                                             detectorName).c_str(),
                                 100, -20, 20, 100, -20, 20);

  hists.hTrueXY = new TH2D("hTrueXY",
                            fmt::format("{} True Hit Positions;x_{{true}} [mm];y_{{true}} [mm]",
                                        detectorName).c_str(),
                            100, -20, 20, 100, -20, 20);

  hists.hRecoXY = new TH2D("hRecoXY",
                            fmt::format("{} Reco Hit Positions;x_{{reco}} [mm];y_{{reco}} [mm]",
                                        detectorName).c_str(),
                            100, -20, 20, 100, -20, 20);

  hists.hResidualVsTrueX = new TH2D("hResidualVsTrueX",
                                     fmt::format("{} X Residual vs True X;x_{{true}} [mm];#Deltax [mm]",
                                                 detectorName).c_str(),
                                     100, -20, 20, 100, -0.5, 0.5);

  hists.hResidualVsTrueY = new TH2D("hResidualVsTrueY",
                                     fmt::format("{} Y Residual vs True Y;y_{{true}} [mm];#Deltay [mm]",
                                                 detectorName).c_str(),
                                     100, -20, 20, 100, -0.5, 0.5);

  // Energy deposit
  hists.hEnergyDeposit = new TH1D("hEnergyDeposit",
                                   fmt::format("{} Energy Deposit;E_{{dep}} [GeV];Counts",
                                               detectorName).c_str(),
                                   100, 0.0, 0.001);
...
void ChargeSharingMonitor::FillData(const std::string& detectorName,
...
void ChargeSharingMonitor::FinishWithGlobalRootLock() {
  ...
  for (const auto& [detName, hists] : m_histograms) {
    if (hists.hResidualX->GetEntries() > 0) {
      ...
      log->info("  X Residual: mean = {:.4f} mm, RMS = {:.4f} mm",
                hists.hResidualX->GetMean(), hists.hResidualX->GetRMS());
      log->info("  Y Residual: mean = {:.4f} mm, RMS = {:.4f} mm",
                hists.hResidualY->GetMean(), hists.hResidualY->GetRMS());
