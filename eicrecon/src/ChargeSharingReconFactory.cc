...
SignalModel toSignalModel(int value) {
    switch (value) {
        case 1:
            return SignalModel::LinA;
        case 0:
        default:
            return SignalModel::LogA;
    }
}
...
ActivePixelMode toActivePixelMode(int value) {
    switch (value) {
        case 1:
            return ActivePixelMode::RowCol;
        case 2:
            return ActivePixelMode::RowCol3x3;
        case 3:
            return ActivePixelMode::ChargeBlock2x2;
        case 4:
            return ActivePixelMode::ChargeBlock3x3;
        case 0:
        default:
            return ActivePixelMode::Neighborhood;
    }
}
...
core::ReconMethod toReconMethod(int value) {
    switch (value) {
        case 0:
            return core::ReconMethod::Centroid;
        case 2:
            return core::ReconMethod::Gaussian2D;
        case 1:
        default:
            return core::ReconMethod::Gaussian1D;
    }
}
...
void ChargeSharingReconFactory::Configure() {
    ...
    if (auto log = logger()) {
        ...
        log->info("  Neighborhood radius: {} ({}x{} grid)", cfg.neighborhoodRadius, 2 * cfg.neighborhoodRadius + 1,
                  2 * cfg.neighborhoodRadius + 1);
        log->info("  Physics: d0={} um, gain={}, ionization={} eV", cfg.d0Micron, cfg.amplificationFactor,
                  cfg.ionizationEnergyEV);
    }
    ...
    if (detector) {
        try {
            ...
            if (!segmentation.isValid()) {
                ...
                if (segImplXY == nullptr && segImplXZ == nullptr) {
                    ...
                    if (decoder == nullptr) {
                        if (auto log = logger()) {
                            log->warn(
                                "Segmentation for readout '{}' lacks a BitField decoder; neighbor bounds unavailable",
                                cfg.readout);
                        }
                    }
                    ...
                    if (auto log = logger()) {
                        log->info("Using DD4hep {} segmentation for readout '{}': pitch=({}, {}) mm, offset=({}, {}) "
                                  "mm, cells=({}, {})",
                                  usingXZCoordinates ? "CartesianGridXZ" : "CartesianGridXY", cfg.readout,
                                  cfg.pixelSpacingMM, cfg.pixelSpacingYMM, cfg.gridOffsetMM, cfg.gridOffsetYMM,
            ...
            if (!detConstants.pixelSize.empty()) {
                try {
                    ...
                    if (std::abs(pixSizeMM - cfg.pixelSizeMM) > 0.001) {
                        ...
                        ++constantsRead;
                        if (auto log = logger()) {
                            log->debug("  Read {} = {} mm (overriding segmentation)", detConstants.pixelSize,
                                       cfg.pixelSizeMM);
                        }
                    }
                } catch (const std::exception&) {
            ...
            if (constantsRead > 0) {
                if (auto log = logger()) {
                    log->info("Read {} DD4hep constants for '{}': Si thickness={} mm, det size={} mm", constantsRead,
                              cfg.readout, cfg.detectorThicknessMM, cfg.detectorSizeMM);
                }
            } else {
                ...
                if (volumeThickness > 0.0) {
                    ...
                    if (auto log = logger()) {
                        log->info("Read Si thickness from detector volume for '{}': {} mm", cfg.readout,
                                  cfg.detectorThicknessMM);
                    }
                } else {
                    // Apply hardcoded fallbacks for detectors without named constants
                    applyDetectorFallbacks(cfg.readout, cfg);
                    if (auto log = logger()) {
                        log->info("Using hardcoded fallbacks for '{}': Si thickness={} mm", cfg.readout,
                                  cfg.detectorThicknessMM);
                    }
                }
            }
    ...
    if (auto log = logger()) {
        ...
        log->info("  Pixel: size={} mm, pitch={} mm, offset={} mm", cfg.pixelSizeMM, cfg.pixelSpacingMM,
                  cfg.gridOffsetMM);
        log->info("  Detector: size={} mm, Si thickness={} mm, pixel thickness={} mm", cfg.detectorSizeMM,
                  cfg.detectorThicknessMM, cfg.pixelThicknessMM);
    }
...
void ChargeSharingReconFactory::Process(int32_t /*runNumber*/, uint64_t /*eventNumber*/) {
    ...
    for (const auto& hit : *simhits) {
        ...
        output->create(hit.getCellID(),          // cellID
