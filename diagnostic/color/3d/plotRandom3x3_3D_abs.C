...
// deltas produced by the 3D fit branches (absolute values), plus their mean.
//
// Usage (from repo root):
//   root -l 'diagnostic/plotRandom3x3_3D.C("epicChargeSharing.root")'
// or simply:
//   root -l diagnostic/plotRandom3x3_3D.C
void plotRandom3x3_3DCore(const char* rootFilePath = "epicChargeSharing.root",
                   const char* outImagePath = "random3x3_3d.svg",
                   bool saveImage = true,
                   Long64_t seed = 0,
                   int forceI0 = -1,
                   int forceJ0 = -1)
{
  ...
  // Pull hits and compute vectors for |DX|/|DY| coloring (3D branches)
...

  {
    ...
    if (!hitsTree) {
      ...
      for (Long64_t i = 0; i < nEntries; ++i) {
        ...
        xs.push_back(x_hit);
        ys.push_back(y_hit);
        const double adx = std::abs(dtx3d);
        const double ady = std::abs(dty3d);
        dxs.push_back(adx);
        dys.push_back(ady);
        dmean.push_back(0.5*(adx+ady));
      }
    }
  }
  ...
  std::string base = outImagePath ? outImagePath : "random3x3_3d.svg";
  std::string ext = ".svg";
  {
  ...
  drawColoredPoints("c_rand3x3_3d_dx", "|#Delta x| [mm]", dxs, dxMin, dxMax, outDx.c_str());
  drawColoredPoints("c_rand3x3_3d_dy", "|#Delta y| [mm]", dys, dyMin, dyMax, outDy.c_str());
  drawColoredPoints("c_rand3x3_3d_dmean", "(|#Delta x|+|#Delta y|)/2 [mm]", dmean, dmMin, dmMax, outDm.c_str());

  f->Close();
  delete f;
}

// Convenience wrappers so running `root -l -b -q plotRandom3x3_3D.C` auto-executes
void plotRandom3x3_3D() {
  ...
  plotRandom3x3_3DCore(defaultPath.c_str());
}

void plotRandom3x3_3D(const char* filename) {
  plotRandom3x3_3DCore(filename, "random3x3_3d.svg", true);
}

// Filename-matching wrapper so `root -l -b -q plotRandom3x3_3D_abs.C` auto-executes
void plotRandom3x3_3D_abs() {
  const char* thisFile = __FILE__;
  std::string thisDir = gSystem->DirName(thisFile);
  std::string defaultPath = thisDir + "/../../../build/epicChargeSharing.root";
  plotRandom3x3_3DCore(defaultPath.c_str());
