...
// Signed variant of plotRandom3x3_3D: colors by signed 3D deltas
// ReconTrueDeltaX_3D_Signed and ReconTrueDeltaY_3D_Signed.
//
// Usage (from repo root):
//   root -l 'diagnostic/plotRandom3x3_3D_signed.C("epicChargeSharing.root")'
// or simply:
//   root -l diagnostic/plotRandom3x3_3D_signed.C
void plotRandom3x3_3DSignedCore(const char* rootFilePath = "epicChargeSharing.root",
                         const char* outImagePath = "random3x3_3d_signed.svg",
                         bool saveImage = true,
                         Long64_t seed = 0,
                         int forceI0 = -1,
                         int forceJ0 = -1)
{
  ...
  // Pull hits and compute vectors for signed |DX|/|DY| coloring (3D signed branches)
...

  {
    ...
    if (!hitsTree) {
      ...
      double dtx3d = 0.0, dty3d = 0.0; // ReconTrueDeltaX/Y 3D (SIGNED)
      ...
      hitsTree->SetBranchStatus("ReconTrueDeltaX_3D_Signed", 1);
      hitsTree->SetBranchStatus("ReconTrueDeltaY_3D_Signed", 1);
      ...
      hitsTree->SetBranchAddress("ReconTrueDeltaX_3D_Signed", &dtx3d);
      hitsTree->SetBranchAddress("ReconTrueDeltaY_3D_Signed", &dty3d);
      ...
      for (Long64_t i = 0; i < nEntries; ++i) {
        ...
        dxs.push_back(dtx3d);
        dys.push_back(dty3d);
        dmean.push_back(0.5*(dtx3d + dty3d));
      }
    }
  }
  ...
  std::string base = outImagePath ? outImagePath : "random3x3_3d_signed.svg";
  std::string ext = ".svg";
  {
  ...
  drawColoredPoints("c_rand3x3_3d_dx_signed", "#Delta x_{3D} (signed) [mm]", dxs, dxMin, dxMax, outDx.c_str());
  drawColoredPoints("c_rand3x3_3d_dy_signed", "#Delta y_{3D} (signed) [mm]", dys, dyMin, dyMax, outDy.c_str());
  drawColoredPoints("c_rand3x3_3d_dmean_signed", "(#Delta x_{3D} + #Delta y_{3D})/2 (signed) [mm]", dmean, dmMin, dmMax, outDm.c_str());

  f->Close();
  delete f;
}

// Convenience wrappers so running `root -l -b -q plotRandom3x3_3D_signed.C` auto-executes
void plotRandom3x3_3D_signed() {
  ...
  plotRandom3x3_3DSignedCore(defaultPath.c_str());
}

void plotRandom3x3_3D_signed(const char* filename) {
  plotRandom3x3_3DSignedCore(filename, "random3x3_3d_signed.svg", true);
}

// Filename-matching wrapper so `root -l -b -q plotRandom3x3_3D_sgn.C` auto-executes
void plotRandom3x3_3D_sgn() {
  const char* thisFile = __FILE__;
  std::string thisDir = gSystem->DirName(thisFile);
  std::string defaultPath = thisDir + "/../../../build/epicChargeSharing.root";
  plotRandom3x3_3DSignedCore(defaultPath.c_str());
