...
// Compare 3D reconstructed deltas against pixel true deltas within a random 3x3 pixel region.
// We compute per-hit:
//   - mean deltas: mean3D = 0.5(|dx3D|+|dy3D|), meanPix = 0.5(|dxPix|+|dyPix|)
//   - diffs: abs and signed for mean, X, and Y
// Only non-pixel hits are drawn.
//
// Usage (from repo root):
//   root -l 'diagnostic/plotRandom3x3_3DvsPixel.C("epicChargeSharing.root")'
// or simply:
//   root -l diagnostic/plotRandom3x3_3DvsPixel.C
void plotRandom3x3_3DvsPixelCore(const char* rootFilePath = "epicChargeSharing.root",
                           const char* outImagePath = "random3x3_3d_vs_pixel.svg",
                           bool saveImage = true,
                           Long64_t seed = 0,
                           int forceI0 = -1,
                           int forceJ0 = -1)
{
...

  {
    ...
    if (!hitsTree) {
      ...
      double dtx3d = 0.0, dty3d = 0.0; // ReconTrueDeltaX/Y (3D, abs)
      ...
      for (Long64_t i = 0; i < nEntries; ++i) {
        ...
        if (!std::isfinite(dpx) || !std::isfinite(dpy) || !std::isfinite(dtx3d) || !std::isfinite(dty3d)) continue;

        xs.push_back(x_hit);
        ys.push_back(y_hit);

        const double mPix = 0.5 * (dpx + dpy);
        const double m3   = 0.5 * (dtx3d + dty3d);

        diffAbsMean.push_back(std::abs(m3 - mPix));
        diffSignedMean.push_back(m3 - mPix);

        diffAbsX.push_back(std::abs(dtx3d - dpx));
        diffSignedX.push_back(dtx3d - dpx);

        diffAbsY.push_back(std::abs(dty3d - dpy));
        diffSignedY.push_back(dty3d - dpy);
      }
    }
  }
  ...
  std::string base = outImagePath ? outImagePath : "random3x3_3d_vs_pixel.svg";
  std::string ext = ".svg";
  {
...
// Convenience wrappers so running `root -l -b -q plotRandom3x3_3DvsPixel.C` auto-executes
void plotRandom3x3_3DvsPixel() {
  ...
  plotRandom3x3_3DvsPixelCore(defaultPath.c_str());
}

void plotRandom3x3_3DvsPixel(const char* filename) {
  plotRandom3x3_3DvsPixelCore(filename, "random3x3_3d_vs_pixel.svg", true);
}
