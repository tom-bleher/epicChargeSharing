...
//   root -l 'diagnostic/plotRandom3x3.C("epicChargeSharing.root")'
// or simply:
//   root -l diagnostic/plotRandom3x3.C
void plotRandom3x3Core(const char* rootFilePath = "epicChargeSharing.root",
                   const char* outImagePath = "random3x3.svg",
                   bool saveImage = true,
                   Long64_t seed = 0,
                   int forceI0 = -1,
                   int forceJ0 = -1)
{
  ...
  // Pull hits and compute vectors for |DX|/|DY| coloring
...

  {
    ...
    if (!hitsTree) {
      ...
      for (Long64_t i = 0; i < nEntries; ++i) {
        ...
        xs.push_back(x_hit);
        ys.push_back(y_hit);
        dxs.push_back(std::abs(dtx));
        dys.push_back(std::abs(dty));
        dmean.push_back(0.5*(std::abs(dtx)+std::abs(dty)));
      }
    }
  }
  ...
  const auto [dxMin, dxMax] = computeRangeAbs(dxs);
  const auto [dyMin, dyMax] = computeRangeAbs(dys);
  const auto [dmMin, dmMax] = computeRangeAbs(dmean);
  ...
  auto drawColoredPoints = [&](const char* cname, const char* zTitle,
    ...
    const double dxW = xMax - xMin;
    const double dyW = yMax - yMin;
  ...
  // Derive output file names: base + _dx.ext and base + _dy.ext
  std::string base = outImagePath ? outImagePath : "random3x3.svg";
  std::string ext = ".svg";
  {
  ...
  std::string outDx = base + "_dx" + ext;
  std::string outDy = base + "_dy" + ext;
  std::string outDm = base + "_dmean" + ext;

  drawColoredPoints("c_rand3x3_dx", "|#Delta x| [mm]", dxs, dxMin, dxMax, outDx.c_str());
  drawColoredPoints("c_rand3x3_dy", "|#Delta y| [mm]", dys, dyMin, dyMax, outDy.c_str());
  drawColoredPoints("c_rand3x3_dmean", "(|#Delta x|+|#Delta y|)/2 [mm]", dmean, dmMin, dmMax, outDm.c_str());

  f->Close();
  delete f;
}

// Convenience wrappers so running `root -l -b -q plotRandom3x3.C` auto-executes
void plotRandom3x3() {
  ...
  plotRandom3x3Core(defaultPath.c_str());
}

void plotRandom3x3(const char* filename) {
  plotRandom3x3Core(filename, "random3x3.svg", true);
}

// Filename-matching wrapper so `root -l -b -q plotRandom3x3_2D_abs.C` auto-executes
void plotRandom3x3_2D_abs() {
  const char* thisFile = __FILE__;
  std::string thisDir = gSystem->DirName(thisFile);
  std::string defaultPath = thisDir + "/../../../build/epicChargeSharing.root";
  plotRandom3x3Core(defaultPath.c_str());
