...
// non-pixel hits that lie inside that rectangle. All geometry is derived from
// ROOT metadata written by the simulation so the placement exactly matches it.
//
// This signed variant uses ReconTrueDeltaX_Signed and ReconTrueDeltaY_Signed.
//
// Usage (from repo root):
//   root -l 'diagnostic/plotRandom3x3_signed.C("epicChargeSharing.root")'
// or simply:
//   root -l diagnostic/plotRandom3x3_signed.C
void plotRandom3x3SignedCore(const char* rootFilePath = "epicChargeSharing.root",
                             const char* outImagePath = "random3x3_signed.svg",
                             bool saveImage = true,
                             Long64_t seed = 0,
                             int forceI0 = -1,
                             int forceJ0 = -1)
{
  ...
  // Pull hits and compute vectors for signed DX/DY coloring
  std::vector<double> xs; xs.reserve(8192);
  std::vector<double> ys; ys.reserve(8192);
  std::vector<double> dxs; dxs.reserve(8192);
  std::vector<double> dys; dys.reserve(8192);
  std::vector<double> dmean; dmean.reserve(8192);

  {
    ...
    if (!hitsTree) {
      ...
      double dtx = 0.0, dty = 0.0; // ReconTrueDeltaX/Y (SIGNED)
      ...
      hitsTree->SetBranchStatus("ReconTrueDeltaX_Signed", 1);
      hitsTree->SetBranchStatus("ReconTrueDeltaY_Signed", 1);
      ...
      hitsTree->SetBranchAddress("ReconTrueDeltaX_Signed", &dtx);
      hitsTree->SetBranchAddress("ReconTrueDeltaY_Signed", &dty);
      ...
      for (Long64_t i = 0; i < nEntries; ++i) {
        ...
        dxs.push_back(dtx);
        dys.push_back(dty);
        dmean.push_back(0.5*(dtx + dty));
      }
    }
  }

  auto computeRangeSigned = [&](const std::vector<double>& v) {
    if (v.empty()) {
      const double M = halfPitch;
      return std::pair<double,double>(-M, M);
    }
    double vmin = v.front();
    double vmax = v.front();
    ...
    const double M = std::max(std::abs(vmin), std::abs(vmax));
    const double Msafe = (M < 1e-12) ? halfPitch : M;
    return std::pair<double,double>(-Msafe, Msafe);
  };

  const auto [dxMin, dxMax] = computeRangeSigned(dxs);
  const auto [dyMin, dyMax] = computeRangeSigned(dys);
  const auto [dmMin, dmMax] = computeRangeSigned(dmean);
  ...
  // Derive output file names: base + _dx_signed.ext and base + _dy_signed.ext
  std::string base = outImagePath ? outImagePath : "random3x3_signed.svg";
  std::string ext = ".svg";
  {
  ...
  std::string outDx = base + "_dx_signed" + ext;
  std::string outDy = base + "_dy_signed" + ext;
  std::string outDm = base + "_dmean_signed" + ext;

  drawColoredPoints("c_rand3x3_dx_signed", "#Delta x (signed) [mm]", dxs, dxMin, dxMax, outDx.c_str());
  drawColoredPoints("c_rand3x3_dy_signed", "#Delta y (signed) [mm]", dys, dyMin, dyMax, outDy.c_str());
  drawColoredPoints("c_rand3x3_dmean_signed", "(#Delta x + #Delta y)/2 (signed) [mm]", dmean, dmMin, dmMax, outDm.c_str());

  f->Close();
  delete f;
}

// Convenience wrappers so running `root -l -b -q plotRandom3x3_signed.C` auto-executes
void plotRandom3x3_signed() {
  ...
  plotRandom3x3SignedCore(defaultPath.c_str());
}

void plotRandom3x3_signed(const char* filename) {
  plotRandom3x3SignedCore(filename, "random3x3_signed.svg", true);
}

// Filename-matching wrapper so `root -l -b -q plotRandom3x3_2d_sgn.C` auto-executes
void plotRandom3x3_2d_sgn() {
  const char* thisFile = __FILE__;
  std::string thisDir = gSystem->DirName(thisFile);
  std::string defaultPath = thisDir + "/../../../build/epicChargeSharing.root";
  plotRandom3x3SignedCore(defaultPath.c_str());
