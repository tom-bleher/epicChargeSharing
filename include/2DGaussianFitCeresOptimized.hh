#ifndef GAUSSIANFITCERESOPTIMIZED2D_HH
#define GAUSSIANFITCERESOPTIMIZED2D_HH

#include <vector>
#include "globals.hh"

// Structure to hold 2D Gaussian fit results (same as original for compatibility)
struct GaussianFit2DResultsCeresOptimized {
    ...
    GaussianFit2DResultsCeresOptimized() : 
...
// Structure to hold diagonal fit results (same as original for compatibility)
struct DiagonalFitResultsCeresOptimized {
    ...
    // Constructor with default values
    DiagonalFitResultsCeresOptimized() : 
...
// OPTIMIZED Function to perform 2D Gaussian fitting using Ceres Solver
// Fits central row and column separately with Gaussian functions
// Function form: y(x) = A * exp(-(x - m)^2 / (2 * sigma^2)) + B
// Uses OPTIMIZED Ceres settings based on comprehensive parameter scan
// 
// OPTIMIZATION RESULTS APPLIED:
// - Initial guess strategy: weighted_centroid (most robust)
// - Parameter bounds: Amplitude factor 10.0x, Center range 3.0 pixel spacings,
//   Sigma range 2.0 pixel spacings, Offset range 2.0x amplitude
// - Solver: DENSE_QR with LEVENBERG_MARQUARDT trust region
// - Tolerances: Function 1e-10, Gradient 1e-10
// - Max iterations: 1500
// - Loss function: NONE (Gaussian data assumption)
// - Outlier filtering: Optional (controlled by parameter)
GaussianFit2DResultsCeresOptimized Fit2DGaussianCeresOptimized(
...
// OPTIMIZED Function to perform diagonal Gaussian fitting using Ceres Solver
...
// Uses OPTIMIZED Ceres settings based on comprehensive parameter scan
DiagonalFitResultsCeresOptimized FitDiagonalGaussianCeresOptimized(
    const std::vector<double>& x_coords,
    const std::vector<double>& y_coords, 
    const std::vector<double>& charge_values,
    double center_x_estimate,
...
#endif // GAUSSIANFITCERESOPTIMIZED2D_HH 