#ifndef LORENTZIANFITCERES3D_HH
#define LORENTZIANFITCERES3D_HH

#include <vector>
#include "globals.hh"

// Structure to hold 3D Lorentzian fit results for entire neighborhood surface
// Function form: z(x,y) = A / (1 + ((x - mx) / γx)^2 + ((y - my) / γy)^2) + B
// where mx, my = center coordinates, γx, γy = widths, A = amplitude, B = baseline
struct LorentzianFit3DResultsCeres {
    ...
    LorentzianFit3DResultsCeres() : 
        center_x(0), center_y(0), gamma_x(0), gamma_y(0), amplitude(0), vertical_offset(0),
...
// Function to perform 3D Lorentzian fitting using Ceres Solver with robust optimization
// Fits entire neighborhood surface with 3D Lorentzian function
// Function form: z(x,y) = A / (1 + ((x - mx) / γx)^2 + ((y - my) / γy)^2) + B
// where mx, my = center coordinates, γx, γy = widths, A = amplitude, B = baseline
