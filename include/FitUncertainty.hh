...
constexpr std::string_view VerticalUncertaintyModelName(VerticalUncertaintyModel model) {
    switch (model) {
...
inline double QnQiScaled(double qi, double qn, double maxQi, double scale = kQnQiDefaultScale) {
...
inline double SelectVerticalSigma(double candidate, double uniformSigma, double fallback = 1.0) {
...
inline double DistancePowerSigma(double distance, double maxCharge, double pixelSpacing, double distanceScalePixels,
...
inline double DistancePowerSigmaInverse(double distance, double maxCharge, double pixelSpacing,
                                        double distanceScalePixels, double exponent, double floorPercent,
                                        double capPercent) {
    if (!std::isfinite(distance) || distance < 0.0) {
        return std::numeric_limits<double>::quiet_NaN();
    }
...
inline double SigmaLinear(double distance, double maxCharge, double pixelSpacing, double distanceScalePixels,
                          double alpha, double floorPercent, double capPercent) {
    if (!std::isfinite(distance) || distance < 0.0) {
        return std::numeric_limits<double>::quiet_NaN();
    }
    if (!std::isfinite(maxCharge) || maxCharge <= 0.0) {
    ...
    const double sigma_raw = (floorPercent / 100.0) * maxCharge * (1.0 + std::max(0.0, alpha) * r);
    return ApplySigmaBounds(sigma_raw, maxCharge, floorPercent, capPercent);
}

inline double SigmaPower(double distance, double maxCharge, double pixelSpacing, double distanceScalePixels,
                         double exponent, double floorPercent, double capPercent) {
    if (!std::isfinite(distance) || distance < 0.0) {
        return std::numeric_limits<double>::quiet_NaN();
    }
    ...
    const double sigma_raw = sigma_min * std::pow(1.0 + std::max(0.0, r), std::max(0.0, exponent));
    return ApplySigmaBounds(sigma_raw, maxCharge, floorPercent, capPercent);
}

inline double SigmaPowerInverse(double distance, double maxCharge, double pixelSpacing, double distanceScalePixels,
                                double exponent, double floorPercent, double capPercent) {
    return DistancePowerSigmaInverse(distance, maxCharge, pixelSpacing, distanceScalePixels, exponent, floorPercent,
                                     capPercent);
}

inline double SigmaQuadratic(double distance, double maxCharge, double pixelSpacing, double distanceScalePixels,
                             double beta, double floorPercent, double capPercent) {
    if (!std::isfinite(distance) || distance < 0.0) {
        return std::numeric_limits<double>::quiet_NaN();
    }
    if (!std::isfinite(maxCharge) || maxCharge <= 0.0) {
    ...
    const double r = distance / scale;
    const double sigma_min = (floorPercent / 100.0) * maxCharge;
    const double sigma_raw = sigma_min * (1.0 + std::max(0.0, beta) * r * r);
    return ApplySigmaBounds(sigma_raw, maxCharge, floorPercent, capPercent);
}

inline double SigmaExpSaturating(double distance, double maxCharge, double pixelSpacing, double distanceScalePixels,
                                 double p, double b, double floorPercent, double capPercent) {
    if (!std::isfinite(distance) || distance < 0.0) {
        return std::numeric_limits<double>::quiet_NaN();
    }
    if (!std::isfinite(maxCharge) || maxCharge <= 0.0) {
    ...
    const double sigma_raw = sigma_min + ((sigma_max - sigma_min) * (1.0 - std::exp(-arg)));
    return ApplySigmaBounds(sigma_raw, maxCharge, floorPercent, capPercent);
}

inline double SigmaPiecewiseCoreTail(double distance, double maxCharge, double pixelSpacing, double distanceScalePixels,
                                     double r0, double beta, double exponent, double floorPercent, double capPercent) {
    if (!std::isfinite(distance) || distance < 0.0) {
        return std::numeric_limits<double>::quiet_NaN();
    }
    if (!std::isfinite(maxCharge) || maxCharge <= 0.0) {
    ...
    if (r > r0) {
        ...
        sigma_raw = sigma_min * (1.0 + std::max(0.0, beta) * std::pow(dr, exponent));
    }
    return ApplySigmaBounds(sigma_raw, maxCharge, floorPercent, capPercent);
}

inline double SigmaLogistic(double distance, double maxCharge, double pixelSpacing, double distanceScalePixels,
                            double r0, double k, double floorPercent, double capPercent) {
    if (!std::isfinite(distance) || distance < 0.0) {
        return std::numeric_limits<double>::quiet_NaN();
    }
    if (!std::isfinite(maxCharge) || maxCharge <= 0.0) {
